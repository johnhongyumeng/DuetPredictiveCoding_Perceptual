...
Figure 7 Modified based on previous oddball paradigm code. Now run this as a seperate function
@author: John Meng
"""
import numpy as np
...
class oddball_model:
    def main(self, dev_degree=270, Tinter=0.5, Tstim=0.5,adap_amount=0.2,
             omission_flag=False, saving_tag='oddball', Tau_int=None, g_IntE=None,Stim_g=None,
...
if __name__=="__main__":
    ...
    #Output_dic=main_model.main(omission_flag=False,saving_tag='oddball_noAdap',adap_amount=0)
    ...
    #Output_dic=main_model.main(Trained_Pathway='\\net\\Output_noHomeoN200.pkl',omission_flag=True, flag_homeo=False,saving_tag='oddballOmissionNoHomeoN200')
    ...
    PltTool.PlotPyrPop(left=-1)
    PltTool.PlotPyrTimes_sample(shift_id=-PltTool.N_eachC//2,top=10)
    PltTool.PlotPyrTimes_sample(shift_id=PltTool.N_eachC//2-1,top=10)
    
    dtheta=360.0/main_model.N_column
    dev_shift= (int(main_model.params_stim['dev_id']/dtheta)-
                int(main_model.params_stim['std_id']/dtheta))*main_model.N_eachC

    PltTool.PlotPyrTimes_sample(shift_id=dev_shift+PltTool.N_eachC//2-1,top=10)
    PltTool.PlotPyrTimes_sample(shift_id=dev_shift-PltTool.N_eachC//2,top=10)

    PltTool.PlotPyrTimes_2samples(shift_id1=PltTool.N_eachC//2-1, shift_id2=dev_shift-PltTool.N_eachC//2,top=10)
    



    MMNshift=PltTool.PlotPyr(shift_id=-PltTool.N_eachC//2)
    MMNshift=PltTool.PlotPyr(shift_id=PltTool.N_eachC//2-1)
    PltTool.PlotPyrDDInd_all()
    
    DD=PltTool.PlotPyrTimes_pop_all()
    PltTool.PlotPyrTimes_pop_RespOrNot()

    PltTool.PlotPyr_conductance(shift_id=-PltTool.N_eachC//2)
    #PltTool.PlotPyr_conductance(shift_id=PltTool.N_eachC//2-1)
    PltTool.PlotPyrTimes_col()
    PltTool.PlotPyrTimes_all_parras()   # New analysis on generating the distribution.
    #DD_value,n_omiss=PltTool.PlotPyrTimes_pop_omission()
