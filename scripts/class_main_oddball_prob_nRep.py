...
Modified based on previous oddball paradigm code. Now run this as a seperate function
@author: John Meng
"""
import numpy as np
...
class oddball_model:
    def main(self, dev_degree=270, Tinter=0.5, Tstim=0.5,adap_amount=0.2,
             omission_flag=False, saving_tag='oddball', Tau_int=None, 
             g_IntE=None,g_back_int=None,Stim_g=None,
        ...
        if Input_blocker is not None:
            time_block=  params_stim['Tresting'] + Input_blocker*(params_stim['Tinter']+params_stim['Tstim']) 
        ...
        stimulus_dev_inc,_=bf.generate_ringblock_stim(params_stim['dev_id'], N_column, N_eachC,
                                                    omission_value*params_stim['str_g']*params_stim['ratio_min'],
                                                    omission_value*params_stim['str_g']*params_stim['ratio_max'],
                                                    params_stim['sigma'])
        ...
        stimulus_dev_ave_sst,_=bf.generate_ringblock_stim(params_stim['dev_id'], N_column, N_eachC,
                                                    omission_value*params_stim['str_g']*params_stim['SST_ratio'],
                                                    omission_value*params_stim['str_g']*params_stim['SST_ratio'],
                                                    params_stim['sigma'])
        ...
        #for i_time in tqdm(range(int(10/params_sim['dt']))):    
            ...
            # Gate
            if (Ind_Gate[i_time]>0.5 or i_time<ind_rest) and i_time<n_block:   # Last one is for blocking.
    ...
    def analyze(self,Plot_flag=True): 
        ...
        PltTool=AT.Tools(self.Output_dic, self.ndt,self.n_frames, self.params_sim['dt'], self.params_stim,
                         time_draw_start=2,Plot_flag=Plot_flag,
                         omission_flag=self.omission_flag, save_dir=save_dir)  # Can add a output directory at the end.
        PltTool.PlotPyrPop()
        PltTool.PlotPVPop()
        PltTool.PlotSSTPop()
        PltTool.PlotIntPop()
        ...
        DD_value,n_omiss=PltTool.PlotPyrTimes_pop_omission()
    ...
    def selectAnalyz(self,Plot_flag=True): 
        ...
        if self.Input_blocker is None:
            ...
            save_dir= ('../figs/blocker/'+self.saving_tag+'/')        
        PltTool=AT.Tools(self.Output_dic, self.ndt,self.n_frames,
                         self.params_sim['dt'], self.params_stim, 
                         time_draw_start=2,Plot_flag=Plot_flag,
                         omission_flag=self.omission_flag, save_dir=save_dir)  # Can add a output directory at the end.
...
if __name__=="__main__":
    # some parameters can be changed here to match auditory detection
    ...
    if Input_blocker is None:
        saving_tag='oddball_prob_nRep'+f'Input_blocker None'
    else:
        saving_tag='oddball_prob_nRep'+f'Input_blocker{Input_blocker:.1f}'
    '''
    main_model.main(Tinter=Tinter, Tstim=Tstim,
                    omission_flag=True, saving_tag=saving_tag,
    ...
    main_model.main(Tinter=Tinter, Tstim=Tstim,
                    omission_flag=True, saving_tag=saving_tag, Tau_int=Tau_int,
                    g_IntE=g_IntE,g_back_int=g_back_int, adap_amount=0,
                    Input_blocker=Input_blocker)
    
    main_model.analyze(Plot_flag=False)

    PltTool=main_model.selectAnalyz()
    #DD, nOmiss=PltTool.PlotPyrPop_omission(columnid=2,left=-0.5)
    MMNshift=PltTool.PlotPyr_omiss(shift_id=-PltTool.N_eachC//2,left=-0.5)
    MMNshift=PltTool.PlotPyr_omiss(shift_id=PltTool.N_eachC//2-1,left=-0.5)
    PltTool.PlotPyrPop(color_scale=3)

    if Input_blocker is not None:
        PltTool.PlotInt_omiss_fit(Input_blocker=Input_blocker,Tinter=Tinter, Tstim=Tstim,Tau_int=Tau_int)
    PltTool.PlotPyrTimes_pop_nRep(nRep=Input_blocker)   # Requires param_stim['choices']
